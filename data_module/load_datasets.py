...
from .csv_dataset import MoleculeCSVDataset,TestDataset
from .smiles_to_dglgraph import smiles_2_dgl


class CCKS22(MoleculeCSVDataset):
    def __init__(self,
                 smiles_to_graph=smiles_2_dgl,
                 cache_file_path='./CCKS22_dglgraph.bin'):
        df = pd.read_csv('../../data/raw/ccks_train.csv')
        super(CCKS22, self).__init__(df=df,
                                     smiles_to_graph=smiles_to_graph,
                                     smiles_column='smiles',
                                     load=True,
                                     cache_file_path=cache_file_path)
    def get_labels(self):
        return torch.flatten(self.labels).tolist()
    def __getitem__(self, item):
        return self.smiles[item], self.graphs[item], self.labels[item], self.mask[item]


class CCKS22Test(TestDataset):
    def __init__(self,
                 smiles_to_graph=smiles_2_dgl,
                 cache_file_path='./CCKS22Test_dglgraph.bin'):
        df = pd.read_csv('../../data/raw/eval_smiles.csv')
        super(CCKS22Test, self).__init__(df=df,
                                     smiles_to_graph=smiles_to_graph,
                                     load=True,
                                     cache_file_path=cache_file_path)
...
class HIV(MoleculeCSVDataset):
    def __init__(self,
        ...
        download(_get_dgl_url(self._url), path=data_path, overwrite=False)
        extract_archive(data_path, dir_path)
        df = pd.read_csv(dir_path + '/_HIV.csv')
